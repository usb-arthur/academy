<template>
    <div>
        <md-table v-model="searched" md-sort="name" md-sort-order="asc" @md-selected="onSelect" md-card md-fixed-header>
            ...
            <md-table-empty-state md-label="Không tìm thấy lĩnh vực"
                :md-description="`Không tìm thấy lĩnh vực tên '${search}'. Thử tìm theo tên khác hoặc tạo mới.`">
                <md-button class="md-primary md-raised" @click="newUser">Tạo lĩnh vực mới</md-button>
            </md-table-empty-state>

            <md-table-row slot="md-table-row" slot-scope="{ item }" md-selectable="multiple" md-auto-select>
                <md-table-cell md-label="ID" md-numeric>{{ item.id }}</md-table-cell>
                <md-table-cell md-label="Catalog" md-sort-by="catalogname">{{ item.catalogname }}</md-table-cell>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            searchByName() {
                if (this.search) {
                    return this.dslinhvucs.filter(item => item.catalogname.toLowerCase().includes(this.search.toLowerCase()))
                }
